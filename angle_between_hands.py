...
def convert_angle_to_degrees_and_minutes(angle: float) -> typing.Tuple[int, int]:
...
def adjust_hours_hand(time: typing.Tuple[int, int]):
...
def find_angle_between_hands(time: typing.Tuple[int, int], realistic_clock: bool = True) -> typing.Tuple[int, int]:
    hour = time[0]
    minute = time[1]
    assert 0 <= hour <= 11, "Hours can only be between 0 and 11"
